...
First create a map for free on umap. Previous versions of this repository only supported https://umap.openstreetmap.fr/en/. With the introduction of parameter `umapDomain`, the domain of umap can be adjusted. After the creation, you include this map by using the openstreetmap shortcode, e.g. `{{< openstreetmap mapName="demo-map_1" >}}` .
...
- `umapDomain` (default `openstreetmap.fr`)
- `coordX` (default `auto`)
- `coordY` (default `auto`)
- `scale`  (default `auto`)
